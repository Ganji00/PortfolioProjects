...
-- likelihood of dying if you contract covid in specific country
Select  location, date, total_cases, total_deaths, (total_deaths/total_cases)*100 AS DeathPercentage
From PortfolioProject..CovidDeaths
WHERE continent is not null-- AND location = 'Poland'
ORDER BY 1, 2

-- Countires with highest DeathPercentage
SELECT location, SUM(new_cases) as total_cases, SUM(CAST(new_deaths AS INT)) as total_deaths, 
(SUM(CAST(new_deaths AS INT))/SUM(new_cases))*100 as MaxDeathPercentage
From PortfolioProject..CovidDeaths
WHERE continent is not null-- AND location = 'Poland'
GROUP BY location
